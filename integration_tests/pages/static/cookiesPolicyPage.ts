...
export default class CookiesPolicyPage extends Page {
  ...
  shouldShowCookiePolicy(): void {
    ...
    cy.get('p').contains('We use cookies to make CAS2 for HDC work.')
    cy.get('h2').contains('Essential cookies')
    cy.get('p').contains(
      'Essential cookies keep your information secure while you use CAS2 for HDC. We do not need to ask permission to use them.',
    )
