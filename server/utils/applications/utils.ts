...
export const createApplicationSummary = (
  ...
  const allocatedPomName = "Joey Jo-Jo Junior Shabadoo"
  const allocatedPomEmail = "joey.jojo@junior.gov.uk"
  const assignmentDate = "2021-06-01"

  const date = assignmentDate
    ? DateFormats.dateObjtoUIDate(new Date(assignmentDate), { format: 'medium' })
    : null

  const pomAllocation = allocatedPomName
    ? `${allocatedPomName}, ${prisonName}`
    : 'To be allocated'
  ...
  const contactEmail = allocatedPomName ? allocatedPomEmail : "rsmith@justice.example.com" //application.createdBy.email
  const emailLabel = allocatedPomName
    ? 'Email address:'
    : 'Offender management unit email address:'
